...
func TestValidateRefuses(t *testing.T) {
	tests := []struct {
		...
		{"address octet over 255", func(r *Record) { r.Cluster.Nodes[0].Address = "10.0.0.256" }, "ktags_cluster.nodes[0].address", ""},
		{"address impossible ipv4", func(r *Record) { r.Cluster.Nodes[0].Address = "999.999.999.999" }, "ktags_cluster.nodes[0].address", ""},
		{"address three octets", func(r *Record) { r.Cluster.Nodes[0].Address = "1.2.3" }, "ktags_cluster.nodes[0].address", ""},
		{"address leading zero octet", func(r *Record) { r.Cluster.Nodes[0].Address = "10.0.0.01" }, "ktags_cluster.nodes[0].address", ""},
		{"address numeric tld", func(r *Record) { r.Cluster.Nodes[0].Address = "node1.example.123" }, "ktags_cluster.nodes[0].address", ""},
		{"jump host impossible ipv4", func(r *Record) { r.Cluster.Access.Jump.Host = "256.1.1.1" }, "ktags_cluster.access.jump.host", ""},
		{"rancher url impossible ipv4", func(r *Record) { r.Cluster.RancherURL = "https://999.1.1.1" }, "ktags_cluster.rancher_url", ""},
...
func TestValidateAcceptsEdgeValues(t *testing.T) {
	tests := []struct {
		...
		{"numeric labels before a named tld", func(r *Record) { r.Cluster.Nodes[0].Address = "10.0.0.1.nip.example" }},
		{"digit-led hostname", func(r *Record) { r.Cluster.Access.Jump.Host = "1jump.example.com" }},
		{"single-label hostname", func(r *Record) { r.Cluster.Nodes[0].Address = "node1" }},
		{"rancher url with ipv4", func(r *Record) { r.Cluster.RancherURL = "https://203.0.113.5" }},
