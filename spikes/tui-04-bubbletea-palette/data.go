...
// upgradeScript builds the fake `ktags upgrade` output for a customer. It only
// exists so the deliberately "extra" registry entry cluster.upgrade has
// something to stream; adding it required no change to any screen.
func upgradeScript(c Customer) []runEvent {
	plays := []playSpec{
		{"drain and upgrade servers", []string{
			"rke2_upgrade : cordon node",
			"rke2_upgrade : drain workloads",
			"rke2_upgrade : install rke2 " + rke2Version,
			"rke2_upgrade : restart rke2-server.service",
			"rke2_upgrade : uncordon node",
		}},
		{"verify", []string{
			"health : nodes_ready",
			"health : node_versions",
