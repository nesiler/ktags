...
type runState struct {
	gen      int
	title    string // health | install | upgrade
	customer string
	cust     Customer // the target, which need not be the focused customer
...
// startRun opens the run view for a `Long` action.
//
// This is the only implementation of the pipeline's "Long" branch: the registry
// entry supplies the title, the target and the fake event script, and every
// long action goes through here. The target does not have to be the customer
// focused on the dashboard (`:health globex` while acme is selected), so the
// run keeps its own copy of the customer for the recap and summary.
func (m *model) startRun(title string, c Customer, events []runEvent) tea.Cmd {
